...
class TestAirfoilOpt(unittest.TestCase):
    ...
    def test_single_point_SLSQP(self):
        os.chdir("singlepoint")
        shutil.copy("../ffd/ffd.xyz", ".")
        shutil.copy("../mesh/n0012.cgns", ".")
        shutil.rmtree("output", ignore_errors=True)
        cmd = ["python", "airfoil_opt.py"]
        subprocess.check_call(mpiCmd + cmd + SLSQP)

    def test_multipoint_SLSQP(self):
        os.chdir("multipoint")
        shutil.copy("../ffd/ffd.xyz", ".")
        shutil.copy("../mesh/n0012.cgns", ".")
        shutil.rmtree("output", ignore_errors=True)
    ...
    @unittest.skipUnless(has_SNOPT, "SNOPT is required for this test")
    def test_single_point_SNOPT(self):
        os.chdir("singlepoint")
        shutil.copy("../ffd/ffd.xyz", ".")
        shutil.copy("../mesh/n0012.cgns", ".")
        shutil.rmtree("output", ignore_errors=True)
    ...
    @unittest.skipUnless(has_SNOPT, "SNOPT is required for this test")
    def test_multipoint_SNOPT(self):
        os.chdir("multipoint")
        shutil.copy("../ffd/ffd.xyz", ".")
        shutil.copy("../mesh/n0012.cgns", ".")
        shutil.rmtree("output", ignore_errors=True)
