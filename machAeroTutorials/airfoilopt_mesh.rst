...
``BC``
    Tells pyHyp that, since it is a 2D problem, both sides of the domain ``jLow`` and ``jHigh``  are set to be symmetry boundary conditions. Note that ``kLow`` is set by default to be a non-slip wall.
``families``
